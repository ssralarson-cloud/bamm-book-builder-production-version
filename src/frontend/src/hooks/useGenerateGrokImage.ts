...
export function useGenerateGrokImage() {
  ...
  const generateImage = async (
    ...
    console.log(
      "[useGenerateGrokImage] Generating image for project:",
      projectId,
    );
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(
          "[useGenerateGrokImage] Grok agent error:",
          response.status,
          errorText,
        );
        throw new Error(
          `Grok agent returned error: ${response.status} ${response.statusText}`,
        );
      }
      ...
      console.log(
        "[useGenerateGrokImage] Image generated successfully:",
        imageUrl,
      );
      ...
      if (err instanceof TypeError && err.message.includes("fetch")) {
        errorMessage =
          "Cannot connect to Grok agent at http://localhost:4001. Please ensure the Grok service is running.";
      } else if (err.message) {
