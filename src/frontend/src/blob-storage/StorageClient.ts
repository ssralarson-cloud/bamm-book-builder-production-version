...
// Module-level cache so image hashes persist across StorageClient instances and page reloads
const globalHashCache: Map<string, string> = new Map();
...
export class StorageClient {
  ...
  public async putFile(
    ...
    globalHashCache.set(path, hashString);
  ...
  public async getDirectURL(path: string): Promise<string> {
    ...
    const hash = this.hashCache.get(path) ?? globalHashCache.get(path);
    if (!hash) {
      throw new Error(
