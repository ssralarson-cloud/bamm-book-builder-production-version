...
import type { Project, KDPValidation, UserProfile, Page } from '../backend';
import { fromBackendProject, type ProjectDTO, logBigIntLeaks } from '../lib/exportUtils';
...
export function toCamelCasePayload<T extends Record<string, any>>(obj: T): T {
  ...
  for (const key in obj) {
    if (Object.prototype.hasOwnProperty.call(obj, key)) {
      ...
      if (value && typeof value === 'object' && !Array.isArray(value) && typeof value !== 'bigint') {
        result[camelKey] = toCamelCasePayload(value);
      } else if (Array.isArray(value)) {
        result[camelKey] = value.map(item => 
          item && typeof item === 'object' && typeof item !== 'bigint' ? toCamelCasePayload(item) : item
        );
      } else {
...
export function normalizeOptionals<T extends Record<string, any>>(obj: T): T {
  ...
  for (const key in obj) {
    if (Object.prototype.hasOwnProperty.call(obj, key)) {
      ...
      else if (value && typeof value === 'object' && !Array.isArray(value) && typeof value !== 'bigint') {
        result[key] = normalizeOptionals(value);
      }
      // Normalize arrays
      else if (Array.isArray(value)) {
        result[key] = value.map(item => 
          item && typeof item === 'object' && typeof item !== 'bigint' ? normalizeOptionals(item) : item === '' ? undefined : item
        );
      }
      else {
        result[key] = value;
  ...
  return useQuery<ProjectDTO[]>({
    ...
    queryFn: async () => {
      return safeCall('listProjects', async () => {
        ...
        console.log('[useProjects/listProjects] Fetching projects from backend');
        const projects = await actorState.actor!.listProjects();
        console.log('[useProjects/listProjects] Successfully fetched', projects.length, 'projects');
...
export function useProject(projectId: string) {
  ...
  return useQuery<ProjectDTO | null>({
    ...
    enabled: actorState.isInitialized && actorState.isAuthenticated && !!projectId,
  });
}
...
export function useCreateProject() {
  ...
  return useMutation({
    mutationFn: async (title: string) => {
      return safeCall('createProject', async () => {
        requireActor(actorState, 'createProject');
        
        console.log('[useCreateProject/createProject] Creating project with title:', title);
        console.log('[useCreateProject/createProject] Backend method: createProject');
        console.log('[useCreateProject/createProject] Timestamp:', new Date().toISOString());
        
        const projectIdNat = await actorState.actor!.createProject(title);
        const projectId = projectIdNat.toString();
        
        console.log('[useCreateProject/createProject] ✓ Project created successfully');
        console.log('[useCreateProject/createProject]   Project ID (Nat):', projectIdNat.toString());
        console.log('[useCreateProject/createProject]   Project ID (string):', projectId);
        console.log('[useCreateProject/createProject]   Timestamp:', new Date().toISOString());
        return projectId;
      }, { title });
    },
    onSuccess: () => {
      console.log('[useCreateProject] Invalidating queries after successful creation');
...
export function useUpdateProject() {
  ...
  return useMutation({
    mutationFn: async (project: Project | ProjectDTO) => {
      return safeCall('updateProject', async () => {
        requireActor(actorState, 'updateProject');
        
        console.log('[useUpdateProject/updateProject] Preparing project for backend:', project.id);
        console.log('[useUpdateProject/updateProject] Backend method: updateProject');
        console.log('[useUpdateProject/updateProject] Pages count:', project.pages.length);
        console.log('[useUpdateProject/updateProject] Timestamp:', new Date().toISOString());
        ...
        console.log('[useUpdateProject/updateProject]   - Cleaned pages:', preparedProject.pages.length);
        console.log('[useUpdateProject/updateProject]   - Normalized optionals: ✓');
        console.log('[useUpdateProject/updateProject]   - CamelCase fields: ✓');
        console.log('[useUpdateProject/updateProject]   - Timestamp (ns):', preparedProject.updatedAt.toString());
        
        await actorState.actor!.updateProject(preparedProject);
        console.log('[useUpdateProject/updateProject] ✓ Project updated successfully');
        console.log('[useUpdateProject/updateProject]   Timestamp:', new Date().toISOString());
      }, { projectId: project.id, pagesCount: project.pages.length });
    },
    onSuccess: (_, project) => {
      console.log('[useUpdateProject] Invalidating queries after successful update');
      queryClient.invalidateQueries({ queryKey: ['projects'] });
      queryClient.invalidateQueries({ queryKey: ['project', project.id] });
      queryClient.invalidateQueries({ queryKey: ['kdp-validation', project.id] });
    },
  });
}
...
export function useDeleteProject() {
  ...
  return useMutation({
    mutationFn: async (projectId: string) => {
      return safeCall('deleteProject', async () => {
        requireActor(actorState, 'deleteProject');
        
        console.log('[useDeleteProject/deleteProject] Deleting project:', projectId);
        console.log('[useDeleteProject/deleteProject] Backend method: deleteProject');
        console.log('[useDeleteProject/deleteProject] Timestamp:', new Date().toISOString());
        
        await actorState.actor!.deleteProject(projectId);
        console.log('[useDeleteProject/deleteProject] ✓ Project deleted successfully');
        console.log('[useDeleteProject/deleteProject]   Timestamp:', new Date().toISOString());
      }, { projectId });
    },
    onSuccess: () => {
      console.log('[useDeleteProject] Invalidating queries after successful deletion');
      queryClient.invalidateQueries({ queryKey: ['projects'] });
    },
  });
}
...
export function useProjectImages(projectId: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      return safeCall('listImages', async () => {
        requireActor(actorState, 'listImages');
        
        console.log('[useProjectImages/listImages] Fetching images for project:', projectId);
        const images = await actorState.actor!.listImages(projectId);
        console.log('[useProjectImages/listImages] ✓ Fetched', images.length, 'images');
        return images;
      }, { projectId });
    },
    enabled: actorState.isInitialized && actorState.isAuthenticated && !!projectId,
...
export function useKDPValidation(projectId: string) {
  ...
  return useQuery<KDPValidation | null>({
    ...
    queryFn: async () => {
      return safeCall('validateKDP', async () => {
        ...
        if (validation && validation.warnings.length > 0) {
          console.log('[useKDPValidation/validateKDP] Warnings:', validation.warnings);
        }
        return validation;
      }, { projectId });
    },
...
export function useAddImage() {
  ...
  return useMutation({
    mutationFn: async ({ projectId, path, dpi }: { projectId: string; path: string; dpi: number }) => {
      return safeCall('addImage', async () => {
        requireActor(actorState, 'addImage');
        
        console.log('[useAddImage/addImage] Adding image:', { projectId, path, dpi });
        console.log('[useAddImage/addImage] Backend method: addImage');
        console.log('[useAddImage/addImage] Timestamp:', new Date().toISOString());
        
        const imageId = await actorState.actor!.addImage(projectId, path, dpi);
        console.log('[useAddImage/addImage] ✓ Image added successfully');
        console.log('[useAddImage/addImage]   Image ID:', imageId);
        console.log('[useAddImage/addImage]   DPI:', dpi, dpi >= 300 ? '(COMPLIANT)' : '(BELOW MINIMUM)');
        console.log('[useAddImage/addImage]   Timestamp:', new Date().toISOString());
        return imageId;
      }, { projectId, path, dpi });
    },
    onSuccess: (_, variables) => {
      console.log('[useAddImage] Invalidating queries after successful image addition');
      queryClient.invalidateQueries({ queryKey: ['images', variables.projectId] });
      queryClient.invalidateQueries({ queryKey: ['kdp-validation', variables.projectId] });
    },
  });
}
...
export function useGenerateImagePrompt() {
  ...
  return useMutation({
    mutationFn: async ({ projectId, pageText, projectContext }: { projectId: string; pageText: string; projectContext: string }) => {
      return safeCall('generateImagePrompt', async () => {
        requireActor(actorState, 'generateImagePrompt');
        
        console.log('[useGenerateImagePrompt/generateImagePrompt] Generating AI prompt for project:', projectId);
        console.log('[useGenerateImagePrompt/generateImagePrompt] Backend method: generateImagePrompt');
        console.log('[useGenerateImagePrompt/generateImagePrompt] Page text length:', pageText.length);
        console.log('[useGenerateImagePrompt/generateImagePrompt] Timestamp:', new Date().toISOString());
        
        const prompt = await actorState.actor!.generateImagePrompt(projectId, pageText, projectContext);
        console.log('[useGenerateImagePrompt/generateImagePrompt] ✓ AI prompt generated successfully');
        console.log('[useGenerateImagePrompt/generateImagePrompt]   Prompt length:', prompt.length);
        console.log('[useGenerateImagePrompt/generateImagePrompt]   Timestamp:', new Date().toISOString());
        return prompt;
  ...
  const query = useQuery<UserProfile | null>({
    ...
    queryFn: async () => {
      return safeCall('getCallerUserProfile', async () => {
        ...
        console.log('[useGetCallerUserProfile/getCallerUserProfile] Fetching user profile');
...
export function useSaveCallerUserProfile() {
  ...
  return useMutation({
    mutationFn: async (profile: UserProfile) => {
      return safeCall('saveCallerUserProfile', async () => {
        requireActor(actorState, 'saveCallerUserProfile');
        
        console.log('[useSaveCallerUserProfile/saveCallerUserProfile] Saving user profile:', profile.name);
        console.log('[useSaveCallerUserProfile/saveCallerUserProfile] Backend method: saveCallerUserProfile');
        console.log('[useSaveCallerUserProfile/saveCallerUserProfile] Timestamp:', new Date().toISOString());
        
        await actorState.actor!.saveCallerUserProfile(profile);
        console.log('[useSaveCallerUserProfile/saveCallerUserProfile] ✓ Profile saved successfully');
        console.log('[useSaveCallerUserProfile/saveCallerUserProfile]   Timestamp:', new Date().toISOString());
      }, { profile });
    },
    onSuccess: () => {
      console.log('[useSaveCallerUserProfile] Invalidating queries after successful profile save');
  ...
  return useQuery<string>({
    ...
    queryFn: async () => {
      return safeCall('getDeploymentUrl', async () => {
        ...
        console.log('[useDeploymentUrl/getDeploymentUrl] Fetching deployment URL');
        console.log('[useDeploymentUrl/getDeploymentUrl] Backend method: getDeploymentUrl');
        console.log('[useDeploymentUrl/getDeploymentUrl] Timestamp:', new Date().toISOString());
        
        const url = await actorState.actor!.getDeploymentUrl();
        console.log('[useDeploymentUrl/getDeploymentUrl] ✓ Deployment URL retrieved:', url);
        console.log('[useDeploymentUrl/getDeploymentUrl]   Timestamp:', new Date().toISOString());
        return url;
      });
    },
