...
import type { Project, KDPValidation } from '../backend';
  ...
  const runTests = async () => {
    ...
    const initialResults: TestResult[] = [
      ...
      },
      {
        ...
        description: 'Verify trim size (8.5×8.5"), bleed (0.125"), and margin specifications',
        status: 'pending',
      },
      {
        step: 'page',
    ...
    try {
      ...
      try {
        ...
        addLog('info', `  Format: 8.5 × 8.5 inches (Amazon KDP standard square)`);
        updateTestResult(0, {
          status: 'passed',
          details: [
      ...
      try {
        ...
        // Validate trim size
        const trimValid = project.cover.dimensions.width === 8.5 && project.cover.dimensions.height === 8.5;
        if (trimValid) {
          formatChecks.push('✓ Trim size: 8.5 × 8.5 inches');
          addLog('success', '✓ Trim size validation passed (8.5 × 8.5 inches)');
        } else {
          formatChecks.push(`✗ Trim size incorrect: ${project.cover.dimensions.width} × ${project.cover.dimensions.height}`);
          formatValid = false;
        ...
        formatChecks.push(`✓ Total dimensions with bleed: ${withBleed} × ${withBleed} inches`);
        addLog('info', `  Total dimensions with bleed: ${withBleed} × ${withBleed} inches`);
      ...
      try {
        ...
        addLog('info', `  Story length: ${updatedProject.story.length} characters`);
      ...
      try {
        ...
        for (const img of testImages) {
          ...
          addLog('info', `  DPI: ${img.dpi} (${img.dpi >= 300 ? 'COMPLIANT' : 'BELOW MINIMUM'})`);
        }
        ...
        addLog('info', `  Minimum required: ${minPixels}×${minPixels} pixels at 300 DPI`);
        addLog('success', `✓ All ${testImages.length} images meet Amazon KDP requirements`);
      ...
      try {
        ...
        for (let i = 0; i < project.pages.length; i++) {
          ...
          const imgPos = page.layout.imagePosition;
          ...
          if (textInSafeZone) {
            ...
            addLog('success', `✓ Page ${i + 1}: Text positioned within safe margins`);
          } else {
            layoutChecks.push(`⚠ Page ${i + 1}: Text may extend beyond safe zone`);
        ...
        layoutChecks.push(`Safe zone dimensions: ${safeZoneSize} × ${safeZoneSize} inches`);
      ...
      try {
        ...
        addLog('info', `  Spine width: ${calculatedSpineWidth.toFixed(4)} inches`);
        addLog('info', `  Formula: ${pageCount} pages × ${spineWidthPerPage}" per page`);
        addLog('info', `  Total cover width: ${coverWidth.toFixed(4)} inches`);
        addLog('info', `  Breakdown: 8.5" (front) + ${calculatedSpineWidth.toFixed(4)}" (spine) + 8.5" (back) + 0.25" (bleed)`);
      ...
      try {
        ...
        if (
          ...
          addLog('success', '✓ Bleed specification compliant (0.125" all sides)');
        } else {
          complianceChecks.push('✗ Bleed specification does not meet requirements');
          isCompliant = false;
        ...
        if (
          ...
          complianceChecks.push('✓ Safe margins: ≥ 0.5" on all sides (COMPLIANT)');
          addLog('success', '✓ Safe margin specification compliant (≥ 0.5" all sides)');
        } else {
        ...
        // Check trim size
        if (validation.trimSize.width === 8.5 && validation.trimSize.height === 8.5) {
          complianceChecks.push('✓ Trim size: 8.5 × 8.5" (COMPLIANT)');
          addLog('success', '✓ Trim size compliant (8.5 × 8.5 inches)');
        } else {
          complianceChecks.push('✗ Trim size does not match Amazon KDP standard');
          isCompliant = false;
        ...
        addLog('error', `✗ Bleed and margin validation failed: ${error.message}`);
      ...
      try {
        ...
        if (validation.warnings.length > 0) {
          complianceChecks.push(`Warnings found: ${validation.warnings.length}`);
        ...
        updateTestResult(7, {
          status: validation.isValid ? 'passed' : validation.errors.length > 0 ? 'failed' : 'warning',
          details: complianceChecks,
        });
        setProgress(80);
      } catch (error: any) {
      ...
      try {
        ...
        addLog('info', `  Validation status: ${validation.isValid ? 'VALID' : 'INVALID'}`);
      ...
      try {
        ...
        const hasValidTrimSize = validation.trimSize.width === 8.5 && validation.trimSize.height === 8.5;
        ...
        const isExportReady = hasValidTrimSize && hasValidBleed && validation.isValid && hasPages;
