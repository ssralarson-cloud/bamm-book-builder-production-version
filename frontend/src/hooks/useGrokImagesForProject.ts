...
import { requestGrokImagesForPages, type GrokImageRequest } from '../lib/grokImageClient';
...
export function useGrokImagesForProject() {
  ...
  const generateGrokImagesForProject = async (project: ProjectDTO) => {
    console.log('[useGrokImagesForProject] Starting image generation for project:', project.id);
    ...
    try {
      ...
      console.log('[useGrokImagesForProject] Found', unillustratedPages.length, 'unillustrated pages');

      // Prepare request for Grok agent
      const grokRequests: GrokImageRequest[] = unillustratedPages.map(page => ({
      ...
      console.log('[useGrokImagesForProject] Received', grokResponses.length, 'images from Grok');
      ...
      const updatedPages = project.pages.map(page => {
        ...
        if (grokResponse) {
          console.log('[useGrokImagesForProject] Updating page', page.pageNumber, 'with image:', grokResponse.imageUrl);
          return {
            ...page,
      ...
      console.log('[useGrokImagesForProject] Saving updated project with', grokResponses.length, 'new images');
      ...
      console.log('[useGrokImagesForProject] Image generation completed successfully');
      setIsGenerating(false);
