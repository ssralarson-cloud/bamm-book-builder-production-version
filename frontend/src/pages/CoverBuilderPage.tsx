...
import { ArrowLeft, Save, Upload, Ruler } from 'lucide-react';
...
import { useProject, useUpdateProject, useKDPValidation, useAddImage } from '../hooks/useQueries';
...
function aspectClassForTrimSize(trimSize: TrimSizePreset): string {
  switch (trimSize) {
    case '8.5x8.5':
      return 'aspect-[1/1]';
    case '8x10':
 ...
 */
function validateCoverImageForKdp(width: number, height: number, coverType: 'front' | 'back'): { warning?: string; dpi: number } {
  ...
  console.log('[validateCoverImageForKdp] Image dimensions:', width, 'x', height, 'pixels');
  console.log('[validateCoverImageForKdp] Target size with bleed:', targetInches, 'x', targetInches, 'inches');
  console.log('[validateCoverImageForKdp] Effective DPI (width):', Math.round(effectiveDpiWidth));
  console.log('[validateCoverImageForKdp] Effective DPI (height):', Math.round(effectiveDpiHeight));
  console.log('[validateCoverImageForKdp] Effective DPI (minimum):', Math.round(effectiveDpi));
  ...
  console.log('[validateCoverImageForKdp] ✓ Image meets 300 DPI recommendation');
...
export default function CoverBuilderPage() {
  ...
  const [dpiWarning, setDpiWarning] = useState<{ front?: string; back?: string }>({});
  const [coverImageUrls, setCoverImageUrls] = useState<{ front?: string; back?: string }>({});
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        img.onload = () => {
          ...
          console.log('[CoverBuilderPage] Image dimensions:', img.naturalWidth, 'x', img.naturalHeight, 'pixels');
          resolve();
        };
        img.onerror = (error) => {
      ...
      const validation = validateCoverImageForKdp(img.naturalWidth, img.naturalHeight, type);
      const effectiveDpi = validation.dpi;
      
      console.log('[CoverBuilderPage] Computed effective DPI:', Math.round(effectiveDpi));
      ...
      await new Promise<string>((resolve, reject) => {
        addImage(
          { projectId, path: imagePath, dpi: effectiveDpi },
          {
            ...
            onError: (error) => {
              console.error('[CoverBuilderPage] ✗ Image registration error:', error);
              reject(error);
            },
          }
        );
      ...
      console.log('[CoverBuilderPage] ✓ Cover preview updated with URL:', uploadedUrl);
      
      // Show success message
      if (!validation.warning) {
        toast.success(`${type === 'front' ? 'Front' : 'Back'} cover uploaded successfully with print-ready resolution (${Math.round(effectiveDpi)} DPI)!`);
      }
      ...
      console.error('[CoverBuilderPage] ═══════════════════════════════════════');
  ...
  const progressSteps = [
    { label: 'Front cover image', completed: !!localProject.cover.front.imageUrl },
    { label: 'Back cover image', completed: !!localProject.cover.back.imageUrl },
    { label: 'Spine text added', completed: localProject.cover.spine.text.length > 0 },
  ...
  return (
    <div className="container py-8">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Design your Amazon KDP print-ready cover with automatic formatting (8.5×8.5" format)</p>
          </div>
        </div>
      ...
      <Alert className="mb-6 border-primary bg-primary/5">
        ...
        <AlertDescription>
          Your cover is automatically configured for Amazon KDP with 8.5×8.5" trim size, 0.125" bleed, and calculated spine width of {spineWidth.toFixed(3)}" based on {localProject.pages.length} pages.
        </AlertDescription>
      </Alert>
            ...
            <TabsContent value="front" className="space-y-4">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Upload an image and add text with automatic safe zone positioning</CardDescription>
                </CardHeader>
            ...
            <TabsContent value="spine" className="space-y-4">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Configure the spine text (width is auto-calculated)</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <Alert>
                    ...
                    <AlertDescription>
                      Based on {localProject.pages.length} pages, your spine width is {spineWidth.toFixed(3)} inches. This is calculated automatically using Amazon KDP's standard formula (0.002252" per page).
                    </AlertDescription>
                  </Alert>
                  <div className="space-y-2">
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Standard 8.5×8.5" square format</CardDescription>
                </div>
                <div className="flex items-center gap-2">
                  <Label htmlFor="show-guides" className="text-xs">Show Guides</Label>
                  <Switch
                    id="show-guides"
                    checked={showGuides}
            ...
            <CardContent className="space-y-4">
              <div className="grid gap-3">
                <div className="flex items-center justify-between rounded-lg border bg-muted/50 p-3">
                  ...
                  <Badge variant="secondary">{TRIM_SIZE}" × {TRIM_SIZE}"</Badge>
                </div>
                <div className="flex items-center justify-between rounded-lg border bg-muted/50 p-3">
                ...
                <div className="flex items-center justify-between rounded-lg border bg-muted/50 p-3">
                  <span className="text-sm font-medium">Total Cover Width:</span>
                  <Badge variant="secondary">{totalCoverWidth.toFixed(3)}"</Badge>
                </div>
                <div className="flex items-center justify-between rounded-lg border bg-muted/50 p-3">
                  <span className="text-sm font-medium">Total Cover Height:</span>
                  <Badge variant="secondary">{totalCoverHeight.toFixed(3)}"</Badge>
                </div>
              </div>
              ...
              <div className="relative aspect-[2/1] rounded-lg border bg-gradient-to-br from-primary/5 to-accent/5 p-2">
                ...
                <div className="flex h-full items-center justify-center gap-1">
                  ...
                  <div 
                    ...
                    style={{ width: `${(spineWidth / totalCoverWidth) * 100}%` }}
                  >
                    <span className="text-[8px] font-semibold text-muted-foreground">SPINE</span>
                  </div>
              ...
              {showGuides && (
                <div className="space-y-1 text-xs">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-muted-foreground">Bleed Zone (0.125")</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="h-3 w-3 border-2 border-dashed border-kdp-trim" />
                    <span className="text-muted-foreground">Trim Line (8.5×8.5")</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="h-3 w-3 border-2 border-dashed border-kdp-safe" />
                    <span className="text-muted-foreground">Safe Zone (0.5" margin)</span>
                  </div>
                </div>
              )}
            </CardContent>
...
function CoverImageUpload({
  ...
  const coverImageUrl = useMemo(() => previewUrl || fileUrl, [previewUrl, fileUrl]);
  ...
  return (
    <div className="space-y-2">
      ...
      />
      <p className="text-xs text-muted-foreground">
        Recommended: 2625×2625 pixels or higher for 300 DPI at 8.75×8.75" (8.5" + 0.125" bleed on each side). Lower resolution images will show a warning but can still be used.
      </p>
    </div>
  );
}
