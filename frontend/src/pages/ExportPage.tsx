...
export default function ExportPage() {
  ...
  const handleExportInterior = async () => {
    ...
    try {
      const pdfBlob = await generateInteriorPDF(project.pages as any[], async (p) => p);
      ...
      console.error('[ExportPage] Interior PDF export failed:', e);
      toast.error('PDF export failed: ' + e.message, { id: 'pdf-export' });
    } finally {
  ...
  const handleExportCover = async () => {
    ...
    try {
      const pdfBlob = await generateCoverPDF(project.cover as any, validation.spineWidth, async (p) => p);
      const url = URL.createObjectURL(pdfBlob);
      const a = document.createElement('a');
      a.href = url;
      a.download = filename;
      ...
      console.error('[ExportPage] Cover PDF export failed:', e);
      toast.error('Cover export failed: ' + e.message, { id: 'cover-export' });
    } finally {
      setIsExporting(false);
    }
  };
