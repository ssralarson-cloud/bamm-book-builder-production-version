...
export default function ExportPage() {
  ...
  useEffect(() => {
    if (!subLoading && !hasSubscription) {
      toast.info('Subscribe to export your book');
      navigate({ to: '/subscribe' });
    }
  }, [subLoading, hasSubscription, navigate]);
  ...
  const handleExportInterior = async () => {
    ...
    try {
      // Build a StorageClient to resolve image paths to fetchable URLs
  ...
  const handleExportCover = async () => {
    ...
    try {
      // Unwrap Candid Opt for cover image URLs
      const frontImageUrl =
            ...
            <CardContent className="space-y-4">
              <div className="rounded-lg bg-muted p-4">
                ...
                <ul className="space-y-1 text-sm text-muted-foreground">
                  ...
                  <li>Trim Size: {validation.trimSize.width}" x {validation.trimSize.height}"</li>
              ...
              <Button
                ...
                disabled={!validation.isValid || isExporting || !actor}
              >
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="rounded-lg bg-muted p-4">
                ...
                <ul className="space-y-1 text-sm text-muted-foreground">
                  <li>Front Cover: {validation.trimSize.width}" x {validation.trimSize.height}"</li>
                  <li>Spine Width: {validation.spineWidth.toFixed(3)}" ({project.pages.length} pages)</li>
                  <li>Back Cover: {validation.trimSize.width}" x {validation.trimSize.height}"</li>
              ...
              <div>
                ...
                <ul className="space-y-1 text-sm text-muted-foreground">
                  <li className={project.pages.length > 0 ? 'text-kdp-success' : ''}>
                    {project.pages.length > 0 ? 'Done' : 'Pending'} Complete all pages
                  </li>
                  <li className={project.pages.some(p => p.imageUrl) ? 'text-kdp-success' : ''}>
                    {project.pages.some(p => p.imageUrl) ? 'Done' : 'Pending'} Add images to pages
                  </li>
                  <li className={project.cover.front.imageUrl || project.cover.back.imageUrl ? 'text-kdp-success' : ''}>
                    {project.cover.front.imageUrl || project.cover.back.imageUrl ? 'Done' : 'Pending'} Design your cover
                  </li>
                  <li className="text-kdp-success">Done - Print format configured</li>
                  <li className={validation.isValid ? 'text-kdp-success' : ''}>
                    {validation.isValid ? 'Done' : 'Pending'} Pass all validations
                  </li>
                </ul>
              </div>
              <Separator />
