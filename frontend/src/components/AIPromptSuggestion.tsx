...
export function AIPromptSuggestion({ projectId, pageText, projectContext, onGenerateImage, isUploading }: AIPromptSuggestionProps) {
  const [aiSuggestion, setAiSuggestion] = useState<string>('');
  const [adjustedPrompt, setAdjustedPrompt] = useState<string>('');
  const [showSuggestion, setShowSuggestion] = useState(false);
  
  const { mutate: generatePrompt, isPending: isGenerating } = useGenerateImagePrompt();
  ...
  const handleSuggestPrompt = () => {
    ...
    console.log('[AIPromptSuggestion] Requesting AI prompt suggestion for text:', pageText.substring(0, 50) + '...');

    generatePrompt(
      { projectId, pageText, projectContext },
      {
        onSuccess: (suggestion) => {
          console.log('[AIPromptSuggestion] AI suggestion received:', suggestion);
          ...
          toast.success('AI prompt suggestion generated!', {
            description: 'Review and adjust the prompt before generating an illustration.',
            duration: 4000,
          });
        },
        onError: (error: any) => {
          ...
          if (error.message.includes('rate limit')) {
            toast.error('Rate limit reached', {
              description: 'Please wait a moment before requesting another suggestion.',
              duration: 5000,
            });
          } else {
  ...
  const handleGenerateIllustration = () => {
    ...
    toast.info('Image generation coming soon', {
      description: 'This feature will generate illustrations using IC Panda AI. For now, please upload an image manually.',
      duration: 6000,
    });
    
    console.log('[AIPromptSuggestion] Would generate image with prompt:', adjustedPrompt);
  };
  ...
  return (
    <Card className="border-primary/20 bg-gradient-to-br from-primary/5 to-transparent">
      <CardHeader>
        ...
        <CardDescription>
          Powered by IC Panda - Get AI-generated prompt suggestions for your page illustrations
        </CardDescription>
      </CardHeader>
          ...
          <div className="space-y-4">
            <Alert className="border-primary/30 bg-primary/5">
              ...
              <AlertDescription className="text-sm">
                AI has analyzed your page text and generated a visual prompt suggestion below.
              </AlertDescription>
            </Alert>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Edit the prompt to add specific details, style preferences, or artistic direction
              </p>
            </div>
            ...
            <p className="text-center text-xs text-muted-foreground">
              Illustration generation powered by IC Panda AI on the Internet Computer
            </p>
          </div>
        )}
      </CardContent>
