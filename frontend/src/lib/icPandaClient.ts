...
export async function generateImagePrompt(pageText: string): Promise<string> {
  console.log('[icPandaClient] generateImagePrompt called - this should use useGenerateImagePrompt hook instead');
  
  // Check rate limiting
  if (!checkRateLimit()) {
    console.warn('[icPandaClient] Rate limit exceeded');
    throw new Error('Rate limit exceeded. Please wait a moment before requesting another suggestion.');
  }
  ...
  if (pageText.length > 5000) {
    console.warn('[icPandaClient] Text too long, truncating to 5000 characters');
    pageText = pageText.substring(0, 5000);
  }
  
  // This function should not be called directly
  // Use the useGenerateImagePrompt hook instead which calls the backend
  throw new Error('Please use the useGenerateImagePrompt hook for AI prompt generation');
}
...
export function generateContextualPrompt(text: string): string {
  ...
  if (lowerText.includes('princess') || lowerText.includes('queen')) characters.push('a princess');
  if (lowerText.includes('prince') || lowerText.includes('king')) characters.push('a prince');
  if (lowerText.includes('witch')) characters.push('a witch');
  if (lowerText.includes('dragon')) characters.push('a dragon');
  if (lowerText.includes('fairy') || lowerText.includes('fairies')) characters.push('a fairy');
  if (lowerText.includes('knight')) characters.push('a knight');
  if (lowerText.includes('child') || lowerText.includes('boy') || lowerText.includes('girl')) characters.push('a child');
  ...
  if (lowerText.includes('rabbit') || lowerText.includes('bunny')) characters.push('a rabbit');
  if (lowerText.includes('bird')) characters.push('a bird');
  
  // Detect settings
  const settings: string[] = [];
  if (lowerText.includes('forest') || lowerText.includes('woods')) settings.push('in an enchanted forest');
  if (lowerText.includes('castle')) settings.push('near a grand castle');
  if (lowerText.includes('village') || lowerText.includes('town')) settings.push('in a quaint village');
  ...
  if (lowerText.includes('sea') || lowerText.includes('ocean')) settings.push('by the sea');
  if (lowerText.includes('sky') || lowerText.includes('cloud')) settings.push('among the clouds');
  if (lowerText.includes('night') || lowerText.includes('moon')) settings.push('under moonlight');
  if (lowerText.includes('day') || lowerText.includes('sun')) settings.push('in golden sunlight');
  
  // Detect mood/atmosphere
  const moods: string[] = [];
  if (lowerText.includes('happy') || lowerText.includes('joy')) moods.push('joyful');
  if (lowerText.includes('sad') || lowerText.includes('cry')) moods.push('melancholic');
  if (lowerText.includes('scary') || lowerText.includes('afraid')) moods.push('mysterious');
  if (lowerText.includes('magic') || lowerText.includes('wonder')) moods.push('magical');
  if (lowerText.includes('adventure')) moods.push('adventurous');
  if (lowerText.includes('peaceful') || lowerText.includes('calm')) moods.push('serene');
  
  // Build the prompt
  let prompt = 'A children\'s book illustration in a black-and-white boho old-world storybook style, inspired by Grimm\'s fairy tales. ';
  ...
  // Add style details
  prompt += 'Line-art style with elegant details, cream and off-white tones, vintage storybook aesthetic. ';
...
export async function generateImage(prompt: string): Promise<Blob> {
  ...
  throw new Error('Image generation not yet implemented. Please upload an image manually.');
}
