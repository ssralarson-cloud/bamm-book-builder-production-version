...
import type { Project, Page, KDPValidation } from '../backend';
 ...
 */
export function toBackendProject(projectDTO: ProjectDTO, ownerPrincipal: Principal): Project {
  return {
    id: projectDTO.id,
...
export function buildExportPayload(
  ...
  console.log('[buildExportPayload] Input project.createdAt type:', typeof project.createdAt);
  console.log('[buildExportPayload] Input project.updatedAt type:', typeof project.updatedAt);
  
  // Ensure timestamps are numbers before converting to ISO strings
  const createdAtMs = typeof project.createdAt === 'number' ? project.createdAt : Number(project.createdAt);
  const updatedAtMs = typeof project.updatedAt === 'number' ? project.updatedAt : Number(project.updatedAt);
  ...
  const normalizedPages = project.pages.map(page => ({
    ...page,
    pageNumber: typeof page.pageNumber === 'number' ? page.pageNumber : Number(page.pageNumber),
  }));
  ...
  console.log('[buildExportPayload] Output payload.project.createdAt type:', typeof payload.project.createdAt);
  console.log('[buildExportPayload] Output payload.project.updatedAt type:', typeof payload.project.updatedAt);
  console.log('[buildExportPayload] Pages:', normalizedPages.length);
  if (normalizedPages.length > 0) {
    console.log('[buildExportPayload] Output payload.project.pages[0].pageNumber type:', typeof normalizedPages[0].pageNumber);
  }
  console.log('[buildExportPayload] Validation:', validation?.isValid ? 'VALID' : 'INVALID');
...
export function exportProjectAsJSON(
  ...
  try {
    ...
    console.log('[exportProjectAsJSON] Export successful, size:', json.length, 'bytes');
    return json;
  } catch (error) {
 ...
 */
export function detectBigIntLeaks(obj: any, path: string = 'root'): string[] {
  const leaks: string[] = [];
  ...
  if (Array.isArray(obj)) {
    obj.forEach((item, index) => {
      leaks.push(...detectBigIntLeaks(item, `${path}[${index}]`));
    });
    return leaks;
  }
